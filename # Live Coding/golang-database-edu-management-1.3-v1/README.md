...
Edu Management API ini digunakan untuk mengakses data dari repositori `student` dan `teacher`. API ini memungkinkan pengguna untuk melakukan berbagai operasi **CRU** _(Create, Read, Update)_ pada data `student` dan **CRD** _(Create, Read, Delete)_ pada data `teacher`.
...
- `repository/teacher.go`
  ...
  - `Delete`: Function ini akan menghapus data guru yang memiliki `id` yang sesuai dengan nilai yang diberikan sebagai argumen. Pertama-tama, function akan mengeksekusi sebuah query `DELETE` untuk menghapus data guru dengan `id` yang sesuai dari tabel `teachers`.
    - Jika proses tersebut berhasil, function akan mengembalikan `nil` sebagai `error`.
    - Namun jika terjadi `error` pada proses tersebut, function akan mengembalikan `error` yang terjadi.
...
2. Function akan mengeksekusi perintah SQL `"INSERT INTO students (name, address, class) VALUES ($1, $2, $3)"` dengan data mahasiswa yang diberikan, yang akan menyimpan data mahasiswa baru ke tabel `students`.
...
2. Function akan mengeksekusi perintah SQL `"SELECT id, name, address, class FROM students"`, yang akan mengambil data seluruh mahasiswa dari tabel `students`.
